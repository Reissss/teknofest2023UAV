...
def localPositionCallback(localPositionCallback):
    ...
    global yaw,pitch,roll,x,y,z,w
    global color_found
    localX = localPositionCallback.pose.position.x
    localY = localPositionCallback.pose.position.y
    x=localPositionCallback.pose.orientation.x
    y=localPositionCallback.pose.orientation.y
    z=localPositionCallback.pose.orientation.z
    w=localPositionCallback.pose.orientation.w
...
def globalPositionPublish(wp_lat, wp_long, wp_alt):
    ...
    while not rospy.is_shutdown ():
        ...
        if ((latitude - 0.000002) < wp_lat < (latitude + 0.000002) and (longitude - 0.000002) < wp_long < (
                longitude + 0.000002) and (amsl - 0.4) < cnt.sp_glob.altitude < (amsl + 0.4))|color_found==True:
            print ("************ ARRIVED ************")
            break
...
class fcuModes:
    ...
    def setArm(self):
        global yaw,first_yaw
        global x,y,z,w,firstx,firsty,firstz,firstw
        firstx=x
        firsty=y
        firstz=z
        firstw=w
...
def moveYaw():
    global firstx,firsty,firstz,firstw,localX,localY
    global altitude1
    print ("************ DONUYOR ************")
    rate = rospy.Rate (10.0)
    angular_p = rospy.Publisher ('/mavros/setpoint_position/local', PoseStamped, queue_size=10)
    msg_angular = PoseStamped ()
    msg_angular.pose.position.x = localX
    msg_angular.pose.position.y = localY
    msg_angular.pose.position.z = altitude1
    msg_angular.pose.orientation.x = firstx
    msg_angular.pose.orientation.y = firsty
    msg_angular.pose.orientation.z = firstz
    msg_angular.pose.orientation.w = firstw
    a=0
    while not rospy.is_shutdown ():
        if(a==15):
            print("DONDU")
...
def moveWaypoint():
    ...
    rate = rospy.Rate (20.0)
    global redLongitude, redLatitude,gordu
    global color_found
    modes = fcuModes ()
    move = Move()
    #globalPositionPublish (40.986077, 28.727398 , 0)
    gordu=1
    globalPositionPublish (40.986077, 28.727398 , 0)
    rospy.sleep(10)
    """move.MoveX(10,-1.5)
